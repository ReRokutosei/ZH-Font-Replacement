...
def _classify_font_files_by_mapping(otf_files, msyh_mapping=None, segoe_mapping=None):
    ...
    if msyh_mapping:
        for _, src in msyh_mapping:
            msyh_sources.add(src.lower())
    
    if segoe_mapping:
        for _, src in segoe_mapping:
            segoe_sources.add(src.lower())
    
    for otf_file in otf_files:
        filename = os.path.basename(otf_file).lower()
        
        # 根据映射分类
        if filename in msyh_sources:
            chinese_fonts.append(otf_file)
        elif filename in segoe_sources:
            english_fonts.append(otf_file)
        else:
            # 如果不在映射中，使用文件名关键词判断
            chinese_keywords = [
                'sourcehanserif', 'sourcehansans', 'sourcehan', 'notosanscjk', 
                'notoserifcjk', 'sarasa', 'simsun', 'simhei', 'microsoftyahei',
                'pingfang', 'hiragino', 'dengxian', 'fangsong', 'kaiti'
            ]
            is_chinese = any(keyword in filename for keyword in chinese_keywords)

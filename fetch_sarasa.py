...
if not isinstance(config, dict):
    raise RuntimeError(
        f"config.yaml 解析异常，返回类型: {type(config)}, 内容: {config}"
    )
        ...
        if file in candidates:
            ...
            if asset:
                ...
                if expected_hash.lower() == actual_hash.lower():
                    ...
                    logging.error(
                        f"本地文件 SHA-256 校验失败: {asset_name}，将自动重新下载"
                    )
                    # 自动重新下载
                    url = None
                    for a in assets:
...
def download(url, save_dir):
    ...
    if asset:
        ...
        if expected_hash.lower() != actual_hash.lower():
            logging.error(
                f"SHA-256 校验失败: {filename} | 预期: {expected_hash} | 实际: {actual_hash}"
            )
            raise ValueError(
                f"SHA-256校验失败: {filename} | 预期值: {expected_hash} | 实际值: {actual_hash}"
            )
